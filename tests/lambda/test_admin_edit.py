...
os.environ['JWT_SECRET_KEY'] = 'test-secret-key'  # テスト用JWT署名キー
...
def test_update_player_success():
    """
    What: 選手編集エンドポイントのテスト（成功ケース）
    Why: personalInfo・bodyCharacteristics変更機能の動作保証
    """
    # DynamoDBテーブル作成（ADR-020: video_id/processed_atキー構造）
    dynamodb = boto3.resource('dynamodb', region_name='us-east-1')
    table = dynamodb.create_table(
    ...
    # admin_editハンドラーをインポート
    from admin_edit.handler import update_player

    # リクエストイベント（personalInfo + bodyCharacteristics変更）
    event = {
        'httpMethod': 'PATCH',
    ...
    # Lambda実行
    response = update_player(event, None)

    # 検証
    assert response['statusCode'] == 200
    body = json.loads(response['body'])
    assert body['success'] is True
    assert body['data']['personalInfo']['firstName'] == '太郎Updated'
    assert body['data']['personalInfo']['birthDate'] == '2010-03-16'
    assert body['data']['bodyCharacteristics']['height'] == 175

    # DynamoDBに保存されたことを確認
    item = table.get_item(Key={'video_id': 'ATHLETE#plr_sakae_19', 'processed_at': 'METADATA'})['Item']
    assert item['personalInfo']['firstName'] == '太郎Updated'
    assert item['personalInfo']['birthDate'] == '2010-03-16'
    # 他のフィールドは変更されていないことを確認
    assert item['personalInfo']['lastName'] == '山田'
    assert item['teamInfo']['jerseyNumber'] == 19


@mock_dynamodb
def test_update_player_not_found():
    """
    What: 選手編集エンドポイントのテスト（選手未存在）
    Why: 404エラーハンドリングの動作保証
    """
    # DynamoDBテーブル作成
    dynamodb = boto3.resource('dynamodb', region_name='us-east-1')
    ...
    event = {
        'httpMethod': 'PATCH',
        'pathParameters': {'playerId': 'plr_nonexistent_99'},
        'body': json.dumps({
            'personalInfo': {
                'firstName': '存在しない'
            }
...
@mock_dynamodb
def test_update_player_forbidden_fields():
    """
    What: 選手編集エンドポイントのテスト（編集不可フィールド）
    Why: jerseyNumber・passwordHash等の変更を明示的に拒否
    """
    # DynamoDBテーブル作成
    dynamodb = boto3.resource('dynamodb', region_name='us-east-1')
    table = dynamodb.create_table(
    ...
    # リクエストイベント（jerseyNumber変更試行）
    event = {
        'httpMethod': 'PATCH',
        'pathParameters': {'playerId': 'plr_sakae_19'},
        'body': json.dumps({
            'teamInfo': {
                'jerseyNumber': 20  # 編集不可
            }
        })
    }
